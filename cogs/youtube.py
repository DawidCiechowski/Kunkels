...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def create_source(cls, ctx, search: str, *, loop, download=False):
        ...
        embed = discord.Embed(title="", description=f"Zakolejkowalem [{data['title']}]({data['webpage_url']}) [{ctx.author.mention}]", color=discord.Color.green())
...
class MusicPlayer:
    ...
    async def player_loop(self):
        ...
        while not self.bot.is_closed():
            ...
            embed = discord.Embed(title="TERAZ NAKURWIAMY", description=f"[{source.title}]({source.web_url}) [{source.requester.mention}]", color=discord.Color.green())
...
class Music(commands.Cog):
    ...
    def __not_connected_message(self) -> discord.Embed:
        return discord.Embed(title="", description="Odpierdol sie, co? Nie gram nigdzie.", color=discord.Color.red())
    
    def __not_playing_message(self) -> discord.Embed:
        return discord.Embed(title="", description="Odpirdol sie, co? Nie gram nic.", color=discord.Color.red())
    
    def __no_channel_message(self) -> discord.Embed:
        return discord.Embed(title="", description="Odpierdol sie! Nie ma mnie nigdzie.", color=discord.Color.red())
    ...
    async def _remove(self, ctx, pos : int=None):
        ...
        if pos == None:
            ...
            try:
                ...
                embed = discord.Embed(title="", description=f"Usunieto [{s['title']}]({s['webpage_url']}) [{s['requester'].mention}]", color=discord.Color.green())
                await ctx.send(embed=embed)
            except:
                embed = discord.Embed(title="", description=f'Nie moglem znalezc "{pos}"', color=discord.Color.red())
                await ctx.send(embed=embed)
    ...
    async def _queue_info(self, ctx):
        ...
        if player.queue.empty():
            embed = discord.Embed(title="", description="Kolejka jest pusta 🖕", color=discord.Color.green())
            return await ctx.send(embed=embed)
        ...
        fmt = '\n'.join(f"`{(upcoming.index(_)) + 1}.` [{_['title']}]({_['webpage_url']}) | ` {duration} Zakolejkowane przez: {_['requester']}`\n" for _ in upcoming)
        ...
        embed = discord.Embed(title=f'Kolejka dla {ctx.guild.name}', description=fmt, color=discord.Color.green())
    ...
    async def _change_volume(self, ctx, *, vol: float=None):
        ...
        if not vol:
            embed = discord.Embed(title="", description=f"🔊 NAKURWIAM NA **{(vc.source.volume)*100}%**", color=discord.Color.red())
            return await ctx.send(embed=embed)

        if not 0 < vol < 101:
            embed = discord.Embed(title="", description="PODAJ LICZBE POMIEDZY 1 A 100, DEBILU", color=discord.Color.red())
            return await ctx.send(embed=embed)
